...
There is no relationship between age and the perceived trauma of being ignored by a parent. There is a relationship between age and the perceived trauma of being ignored by a parent.

```{r}
library(tidyverse)

# Check the variables first (optional but helpful)
str(df$Age)
str(df$`was ignored by a parent`)
...
nrow(df_cor)   # Just to see how many valid cases you have
```

```{r}
# Pearson correlation between Age and trauma rating
cor.test(
  df_cor$Age,
  df_cor$`was ignored by a parent`,
  method = "pearson"
)

```

This testes my hypothesis: Is there an association between age and how traumatic participants rate being ignored by a parent?
